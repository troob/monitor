...
def delete_file_path(file_path, data_type=''):
	# print('\n===Delete File Path===\n')
	# print('file_path: ' + file_path) # data/ev data todays date.csv
...
# remove items with keyword
# if no key field then checks all fields for keyword
def remove_from_json(remove_key, file, key_field_name=''):

	init_dict = reader.read_json(file)

	final_dict = {}

	final_id = 0
	for dict in init_dict.values():
		#print('dict: ' + dict)
		
		key_field = dict[key_field_name]
		if not re.search(remove_key.lower(), key_field.lower()):
			final_dict[final_id] = dict
			final_id += 1


	# === Remove Dash from Player Names === 
	# for game_key, box_score in init_dict.items():
	...
	# make new copy as backup? not needed bc manual backup first
	
	
	writer.write_json_to_file(final_dict, file)
...
remove_key = 'Baylor University vs Iowa State University'
file = 'data/arbs.json'
key_field_name = 'game'
remove_from_json(remove_key, file, key_field_name)
